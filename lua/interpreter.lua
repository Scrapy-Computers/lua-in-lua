function createInterpreter( settings )
    local m = {}
    m.settings = {
        realtime = true
    }
    if settings.realtime ~= nil then m.settings.realtime = settings.realtime end --lua keeps defaulting to true in a terenary operator
    ...
    m.evals = {
        ...
        ["while"] = function(self, node, environment)
            ...
            if self.settings.realtime then while not loop() do end
            else table.insert(self.loops, loop) end
        end,
        ...
        ["for"] = function(self, node, environment)
            ...
            local i = start
            local function loop() 
                if i > end_loop then return true end
...
lua_interpreter = createInterpreter